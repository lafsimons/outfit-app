...
## MBA corrupted files — Critical rename/media integrity fix

Highest priority.

The MBA rename/title save path is still corrupting blob-backed imported media references.

Current observed behavior:

* Renaming/editing title on existing imported references can cause:
    * missing preview/media
    * orphaned preview rows
    * duplicated preview assets
* Re-adding the image manually repairs the item, but leaks orphaned media rows.

Current architecture state:

* Libraries are now normalized into blob-backed imported previews after scalable export/import.
* Integrity checks are otherwise healthy.
* The remaining corruption vector is rename/title metadata saves.

Please implement a robust metadata-only save architecture.

Requirements:

1. Separate metadata concepts clearly:

* immutable internal item ID
* display title (editable)
* original imported filename (immutable provenance)
* optional current external filename / aliases
* media storage keys

2. Metadata-only edits must:

* preserve all blob-backed media references
* preserve preview asset IDs
* preserve original blob references
* preserve thumbnails/original metadata
* never regenerate or replace media unless a new image was explicitly selected

3. Rename/title edits must NOT:

* create new preview assets
* create orphaned preview rows
* duplicate media groups
* detach existing media

4. Add orphan/media cleanup:

* if media replacement actually occurs, garbage-collect detached preview rows safely

5. Add regression tests:

* rename imported blob-backed item
* rename + tag edit
* repeated metadata edits
* image replacement flow
* scalable-imported items
* ensure preview asset count remains stable

6. Add temporary debug logging:
    before/after media-bearing fields during save.
...
Implement rolling metadata autosnapshots for MBA/OA safety and recovery.

Goal:
Prevent loss of tagging/ontology/metadata work.

Implement:

1. Metadata snapshot store

* IndexedDB metadataSnapshots store
* metadata only (no media blobs)
* include:
    * items metadata
    * boards metadata
    * app settings
...
3. Rolling autosnapshots
    Create snapshots:

* every 15–30 minutes if dirty
* on tab hide/close
...
* keep last 30–50 snapshots
* automatically prune oldest

5. Archive status UI
    Show:

* last metadata autosave
* last full backup
* changed items since backup
