...
export default function WardrobeSelectionBar({
  ...
  function handleMenuAction(action) {
    ...
    setActiveAction(null);
    setMenuOpen(false);
    if (action.closeEditor !== false) {
      onCloseEdit?.();
    }
  }

  const hasCustomMenuItems = Array.isArray(customMenuItems) && customMenuItems.length > 0;
  const hasCustomDangerMenuItems = Array.isArray(customDangerMenuItems) && customDangerMenuItems.length > 0;
  const usesCustomMenu = hasCustomMenuItems || hasCustomDangerMenuItems;
                  ...
                  <>
                    {hasCustomMenuItems ? (
                      <div className="wardrobe-selection-menu-section">
                        {customMenuItems.map((action) => (
                          <button
                            key={action.label}
                            type="button"
                            className={`ghost-button wardrobe-selection-menu-button ${action.isActive ? "is-active" : ""}`.trim()}
                            onMouseDown={preventPointerFocus}
                            onClick={() => handleMenuAction(action)}
                            disabled={!selectedCount || action.disabled}
                          >
                            {action.label}
                          </button>
                        ))}
                    ...
                    {hasCustomDangerMenuItems ? (
                      <div className={`wardrobe-selection-menu-section ${hasCustomMenuItems ? "wardrobe-selection-menu-section-danger" : ""}`.trim()}>
                        {customDangerMenuItems.map((action) => (
                          <button
                            key={action.label}
                            type="button"
